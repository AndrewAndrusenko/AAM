-- FUNCTION: public.f_f_get_management_fees_schedules(text[], date, date)

-- DROP FUNCTION IF EXISTS public.f_f_get_management_fees_schedules(text[], date, date);

CREATE OR REPLACE FUNCTION public.f_f_get_management_fees_schedules(
	p_portfolio_list text[],
	p_report_date_start date,
	p_report_date_end date)
    RETURNS TABLE(object_id bigint, portfolioname character varying, id_fee integer, fee_code character varying, fee_type numeric, calc_start date, calc_end date, period_start date, period_end date, fee_type_value smallint, feevalue numeric, calculation_period numeric, schedule_range numrange, range_parameter character varying, below_ranges_calc_type smallint) 
    LANGUAGE 'plpgsql'
    COST 100
...
ALTER FUNCTION public.f_f_get_management_fees_schedules(text[], date, date)
    OWNER TO postgres;
