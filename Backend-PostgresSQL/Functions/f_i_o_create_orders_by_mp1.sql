...
DROP FUNCTION IF EXISTS public.f_i_o_create_orders_by_mp1(text[], text[], date, integer, numeric);

CREATE OR REPLACE FUNCTION public.f_i_o_create_orders_by_mp1(
...
f_cm_raise_exception ('There are active opposite orders for the following  '||(SELECT array_agg(concat(portfolioname, ' ',secid,' ',"type",'. Opposite order:',reverse_orders_select.id)) from reverse_orders_select)::text,TRUE) END	
from new_orders;
...
ALTER FUNCTION public.f_i_o_create_orders_by_mp1(text[], text[], date, integer, numeric)
    OWNER TO postgres;
SELECT * FROM
f_i_o_create_orders_by_mp1 (array['acm002','vpc005','icm011'],array['aapl-rm'],now()::date,840,0.01)
