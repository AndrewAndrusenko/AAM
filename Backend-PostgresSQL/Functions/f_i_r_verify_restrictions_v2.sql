...
WITH
  ...
  vr AS (
    SELECT
      ...
      res.mp_name,
      res.mp_id,
			CASE WHEN res.rest_type='cash' THEN -1 ELSE 1 END AS cash_flag,
      CASE
  ...
  main_set AS (
    SELECT
      ...
      CASE
        WHEN pd.mtm_postion*vr.cash_flag / pd.npv * 100 < vr.portfolio_restriction_by_mp THEN 0
        ELSE ROUND(pd.mtm_postion*vr.cash_flag / pd.npv * 100, 4) - vr.portfolio_restriction_by_mp
...
ALTER FUNCTION public.f_i_r_verify_restrictions_v2(text[])
    OWNER TO postgres;
select * from f_i_r_verify_restrictions_v2(array['vpc005'])
