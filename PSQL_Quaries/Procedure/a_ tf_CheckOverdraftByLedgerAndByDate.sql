...
CREATE OR REPLACE FUNCTION public.stf_CheckOverdraftByLedgerAndByDate(
    ...
    RETURNS TABLE
	( "openingBalance" money,  "accountTransaction" money, "CrSignAmount" money, "DbSignAmount" money
...
select MAX("IncomingBalances"."closingBalance") ,
MAX("accountTransactionTotals"."sumAccountTransactions"),

CAST(SUM (CASE ( "bcAccountType_Ext"."xActTypeCode")
	WHEN 1  THEN "amount" *-1
	WHEN 2 THEN "amount" 
END) FILTER (WHERE "bLedgerTransactions"."ledgerID"=$2 AND
	"id" !=$5 AND 
