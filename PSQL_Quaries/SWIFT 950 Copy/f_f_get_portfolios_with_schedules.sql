...
-- DROP FUNCTION IF EXISTS public.f_f_get_portfolios_with_schedules(numeric, numeric);

CREATE OR REPLACE FUNCTION public.f_f_get_portfolios_with_schedules(
	p_object_id numeric,
	p_id_fee_main numeric)
    RETURNS TABLE(portfolioname character varying, id_fee integer, fee_code character varying, fee_type_desc character varying, fee_object_desc character varying, fee_description character varying, period_desc character varying, fee_type numeric, main_fee_object_type smallint, id_fee_period numeric, id integer, object_id bigint, id_fee_main numeric, period_start date, period_end date, created time without time zone, modified time without time zone, idportfolio integer) 
    LANGUAGE 'plpgsql'
    COST 100
  ...
  dfees_main.id as id_fee,
   ...
   FROM dfees_main
	 ...
	 LEFT JOIN dportfolios ON dportfolios.idportfolio = dfees_objects.object_id
	  WHERE 
		(p_object_id ISNULL OR  dfees_objects.object_id = p_object_id)  AND  
		(p_id_fee_main ISNULL OR  dfees_objects.id_fee_main = p_id_fee_main) AND
		dportfolios.idportfolio NOTNULL
