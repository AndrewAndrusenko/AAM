...
-- DROP FUNCTION IF EXISTS public.f2_orders_allocation(	trade_qty numeric,orders_for_allocation numeric[]);

CREATE OR REPLACE FUNCTION public.f2_orders_allocation(
	trade_qty numeric,
    orders_for_allocation numeric[])
    RETURNS TABLE(id bigint , allocated_qty numeric, corrected_qty numeric,id_portfolio numeric,parent_order integer,ratio numeric) 
    LANGUAGE 'plpgsql'
    COST 100
...
WITH unexecuted_orders AS ( 
	SELECT dorders.id, dorders.generated, dorders.id_portfolio, dorders.secid, dorders.qty - COALESCE(allocation.allocated,0) as unexecuted, dorders.qty,allocation.allocated, dorders.parent_order
	FROM public.dorders 
	...
	SELECT unexecuted_orders.id, unexecuted_orders.generated, unexecuted_orders.id_portfolio, unexecuted_orders.secid, 
...
ALTER FUNCTION public.f2_orders_allocation(
	trade_qty numeric,
    orders_for_allocation numeric[])
    OWNER TO postgres;
