...
SELECT  "accountNo", "accountId", "accountType", "datePreviousBalance" , COALESCE("dateBalance"::date ,$2::date) AS "dateBalance" , 
	...
	FROM f_a_b_balances_closed_all()
	where "dateBalance"::date = $1::date
UNION
SELECT COALESCE(f_a_b_balances_closed_all."accountNo",turnovers."accountNo") AS "accountNo", 
     COALESCE(f_a_b_balances_closed_all."accountId", turnovers."accountId") AS "accountId", 
	 'Entries' AS "accountType",
     COALESCE(f_a_b_balances_closed_all."datePreviousBalance", $2::date) AS  "datePreviousBalance",
	 COALESCE(f_a_b_balances_closed_all."dateBalance", $2::date) AS "dateBalance",
	 COALESCE( f_a_b_balances_closed_all."openingBalance", 0)::numeric AS "openingBalance",
	 COALESCE(turnovers."totalDebit",0), COALESCE(turnovers."totalCredit",0), 
	 COALESCE(turnovers."outBalance", f_a_b_balances_closed_all."openingBalance"), 0, f_a_b_balances_closed_all."xacttypecode" 
FROM (SELECT  f_a_b_balances_closed_all."accountNo", f_a_b_balances_closed_all."accountId", f_a_b_balances_closed_all."accountType", 
	  f_a_b_balances_closed_all."datePreviousBalance" ,f_a_b_balances_closed_all."dateBalance" , f_a_b_balances_closed_all."openingBalance",f_a_b_balances_closed_all."xacttypecode"  
	  FROM f_a_b_balances_closed_all()
	  WHERE f_a_b_balances_closed_all."dateBalance"::date = $2::date
	  ) AS f_a_b_balances_closed_all
	FULL OUTER JOIN (
		...
		COALESCE(MAX("openingBalance") + 
		 ...
		 FROM public.f_a_b_current_turnovers_and_balnces_not_closed($2::date) 
		...
		COALESCE (MAX("openingBalance") + 
		SUM("signedTurnOver") FILTER (WHERE "dataTime"::date !=$2::date),MAX ("openingBalance")) as "outBalance", 0,"xActTypeCode"
		 FROM public.f_a_b_bcurrent_ledger_turnovers_balances_notclosed($2::date) 
		where "dataTime"::date <=$1::date
		group by "accountId", "accountNo", "xActTypeCode"
	) as turnovers
	ON f_a_b_balances_closed_all."accountNo" = turnovers."accountNo"
