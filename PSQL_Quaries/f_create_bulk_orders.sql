...
RETURN query
WITH bulk_orders as (
	INSERT INTO dorders (type, secid, qty, price, amount, status,  order_type, idcurrency, child_orders,ordertype, mp_id)
		SELECT dorders.type, dorders.secid, SUM(dorders.qty) as qty, dorders.price, 
			   SUM(dorders.amount) AS amount, dorders.status, dorders.order_type, dorders.idcurrency, array_agg(dorders.id) AS child_array,'Bulk',dorders.mp_id
		FROM public.dorders 
		WHERE dorders.id = ANY ($1) AND parent_order ISNULL 
		GROUP BY dorders.type, dorders.secid, dorders.price,dorders.idcurrency,dorders.status,dorders.order_type,dorders.mp_id
	RETURNING *
    ), client_orders AS (
