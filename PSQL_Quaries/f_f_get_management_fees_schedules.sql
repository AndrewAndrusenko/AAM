...
CREATE OR REPLACE FUNCTION public.f_f_get_management_fees_schedules(
    ...
    RETURNS TABLE(
		...
		feevalue numeric,
		calculation_period numeric,
		schedule_range numrange  ,
		range_parameter char varying,
		below_ranges_calc_type smallint )
    LANGUAGE 'plpgsql'
    COST 100
...
ALTER FUNCTION public.f_f_get_management_fees_schedules(text[],date, date)
    OWNER TO postgres;
select * from f_f_get_management_fees_schedules(array(select portfolioname from dportfolios),(now() - '1 months'::interval)::date,now()::date)