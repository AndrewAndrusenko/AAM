...
CREATE OR REPLACE FUNCTION public.f_i_get_accured_interests_for_portfolios(
    ...
    RETURNS TABLE(
		...
		coupon_calc numeric, 
		couponrate numeric, 
		couponamount numeric, 
		unredemeedvalue numeric, 
		facevalue numeric, 
		faceunit character varying, 
...
WITH instruments_list as (
	...
	FROM
	  mmoexsecurities
	  LEFT JOIN mmoexsecuritytypes ON mmoexsecurities."group"=mmoexsecuritytypes.security_group_name
...
SELECT
  ...
  ROUND((instruments_list.facevalue * mmoexcorpactions.couponrate / 100 * 
   (EXTRACT(days FROM coupons_info.days_qty) -1) / 365)
