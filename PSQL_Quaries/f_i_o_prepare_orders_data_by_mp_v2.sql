...
DROP FUNCTION IF EXISTS public.f_i_o_prepare_orders_data_by_mp_v2(int,text[], text[], date, integer, numeric);
...
CREATE OR REPLACE FUNCTION public.f_i_o_prepare_orders_data_by_mp_v2(
    ...
    RETURNS table ( 
		...
		balance_net numeric,
		balance_net_with_orders numeric,
		...
		orders_net_amount_rt numeric,
		net_orders numeric,
		portfolio_code char varying,portfolio_leverage_by_mp numeric,
	  mp_leverage_weight numeric, leverage_amount numeric, leverage_handle int,leverage numeric, balance numeric,
		id bigint,generated date,order_type numeric,idcurrency numeric,child_orders int[],mp_id bigint,qty numeric,price numeric,amount numeric,qty_executed numeric,
		parent_order integer,id_portfolio numeric,type character varying,secid character varying,status character varying,ordertype character varying) 
    LANGUAGE 'plpgsql'
    COST 100
...
main_set AS (
	...
	CASE WHEN leverage_restrictions.mp_leverage_weight<= leverage_restrictions.portfolio_leverage_by_mp THEN true ELSE false END AS mp_leverage_good,
		...
		unacc_orders.net_orders ::numeric(20,2),
	...
	LEFT JOIN LATERAL (
		SELECT   rt.mp_id,rt.mp_name,rt.mp_weight, rt.mp_leverage_weight, rt.portfolio_leverage_by_mp,rt.leverage_restrction
		FROM f_i_model_portfolios_get_leverage (
		ARRAY(SELECT DISTINCT new_orders.id_portfolio FROM new_orders)) rt
		WHERE rt.id=orders.id_portfolio AND rt.mp_id=orders.mp_id
	...
	WHERE
		LOWER(orders.secid) = ANY (p_secid_list)
		AND ABS(orders.amount / orders.notnull_npv * 100) > p_min_deviation
		AND orders.qty>0
),
...
WHERE (	
	CASE 
		...
		WHEN p_leverage_handle = 2 THEN main_set_with_maxlimit.order_at_max_limit
	END
	*100/main_set_with_maxlimit.npv)>p_min_deviation 
;
END;
$BODY$;

ALTER FUNCTION public.f_i_o_prepare_orders_data_by_mp_v2(int,text[], text[], date, integer, numeric)
    OWNER TO postgres;

select order_amount_main*100/npv, price,portfolio_code, secid,amount != order_amount_main as chg, amount,* 
from f_i_o_prepare_orders_data_by_mp_v2 
(2,array['acm002'],array['lly-rm','nflx-rm','tsla-rm','aapl-rm','csco-rm','spot-rm'],now()::date,840,0.1)
