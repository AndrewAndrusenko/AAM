-- FUNCTION: public.f_i_model_portfolios_get_leverage(numeric[])

-- DROP FUNCTION IF EXISTS public.f_i_r_get_leverage_restrictions(numeric[]);

CREATE OR REPLACE FUNCTION public.f_i_r_get_leverage_restrictions(
	p_idportfolios numeric[])
    RETURNS TABLE(mp_id integer, id integer, code character varying, mp_name character varying, mp_weight numeric, mp_leverage_weight numeric, portfolio_leverage_by_mp numeric, leverage_restrction numeric) 
    LANGUAGE 'plpgsql' 
    COST 100
...
ALTER FUNCTION public.f_i_r_get_leverage_restrictions(numeric[])
    OWNER TO postgres;
