...
export class AppaIAccFeesManagementTableComponent {
  ...
  submitQuery (reset:boolean=false, showSnackResult:boolean=true) {
    ...
    of(this.portfolios.length).pipe(
      ...
      switchMap(ports=>this.AppFeesHandlingService.getManagementFeeCalcData(searchObj))
    ).subscribe(data => {
  ...
  // }
  exportToExcel() {
    let numberFields=['npv','management_fee_amount','feevalue','cross_rate','dirty_price','fee_type'];
    let dateFields=['report_date','calculation_start','calculation_end','period_start','period_end'];
    let dataToExport =  structuredClone(this.fullDataSource);
    dataToExport.map(el=>{
      Object.keys(el).forEach(key=>{
        switch (true==true) {
          case el[key]&&numberFields.includes(key): return el[key]=Number(el[key]) ;
          case el[key]&&dateFields.includes(key): return el[key]=new Date(el[key])
